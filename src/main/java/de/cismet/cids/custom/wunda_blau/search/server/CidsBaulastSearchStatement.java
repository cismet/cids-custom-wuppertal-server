...
public class CidsBaulastSearchStatement extends AbstractCidsServerSearch implements MetaObjectNodeServerSearch {
    ...
    private String getSecondaryQuery() {
        ...
        final String queryMid = ""
                    ...
                    + "\n                      AND CASE WHEN NOT st_isEmpty(y.geo_field) AND NOT st_isEmpty(g.geo_field) THEN Intersects(St_buffer(y.geo_field, 0.000001), St_buffer(g.geo_field, 0.000001)) ELSE FALSE END "
                    + "\n                      AND NOT y.fid = f.id "
                    + "\n                      AND CASE WHEN NOT st_isEmpty(y.geo_field) AND NOT st_isEmpty(g.geo_field) THEN Intersects(st_Buffer(y.geo_field, -0.05), St_buffer(g.geo_field, 0.000001)) ELSE FALSE END) AS indirekt "
                    + "\n                                 WHERE  1 = 1 "
                    + "\n                                        AND l.id = fsj.baulast_reference "
                    + "\n                                        AND fsj.flurstueck = k.id "
                    + "\n                                        AND k.fs_referenz = f.id "
